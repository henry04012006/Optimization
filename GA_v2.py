...
K = 0
SIZE = 0
D: List[int] = []                  # Flat distance matrix: D[i * SIZE + j]
d: List[List[int]] = []            # Kept only for compatibility/debugging
...
def get_config(n: int, k: int) -> Dict[str, Any]:
    # Basic GA only: no 2-opt, no memetic improve(), no final local search.
    if n <= 250:
        return dict(
            mode="small", pop_size=78, elite=5, time_limit=28.75,
            crossover_rate=0.92, mutation_rate=0.30,
            no_improve_limit=34, replace_div=6,
            route_limit=k, init_route_limit=k,
            tournament_size=3, reserve_time=0.12,
            relocate_sample=18, relocate_dest=min(7, max(1, k - 1)), mixed=True,
            copy_route_bias=0.70, order_crossover_rate=0.18,
        )

    if n <= 600:
        return dict(
            mode="medium", pop_size=52, elite=4, time_limit=28.75,
            crossover_rate=0.86, mutation_rate=0.30,
            no_improve_limit=28, replace_div=6,
            route_limit=min(8, k), init_route_limit=min(8, k),
            tournament_size=3, reserve_time=0.12,
            relocate_sample=13, relocate_dest=min(5, max(1, k - 1)), mixed=True,
            copy_route_bias=0.65, order_crossover_rate=0.20,
        )

    return dict(
        mode="large", pop_size=34, elite=2, time_limit=28.75,
        crossover_rate=0.78, mutation_rate=0.34,
        no_improve_limit=24, replace_div=5,
        route_limit=min(5, k), init_route_limit=min(4, k),
        tournament_size=3, reserve_time=0.15,
        relocate_sample=8, relocate_dest=min(4, max(1, k - 1)), mixed=False,
        copy_route_bias=0.60, order_crossover_rate=0.18,
    )
...
def evaluate_clean(routes: List[List[int]], costs: Optional[List[int]] = None) -> Individual:
    # Use only when routes are already feasible and internally closed.
    if costs is None:
...
def repair_routes(routes: List[List[int]]) -> List[List[int]]:
    ...
    if missing:
        ...
        missing.sort(key=lambda x: D[x], reverse=True)  # distance from depot
        for x in missing:
...
def perturb_routes(routes: List[List[int]], moves: int) -> List[List[int]]:
    ...
    random_relocate_inplace(routes, moves)
    return routes
...
def initial_population(size: int, cfg: Dict[str, Any], end_time: float) -> List[Individual]:
    ...
    for i in range(size):
        ...
        mode = i % 3
        noise = 0.0 if i < 3 else 0.015 * (1 + (i % 4))
...
def guided_relocate_from_longest(routes: List[List[int]], costs: List[int],
                                 ...
                                 mixed: bool) -> bool:
    # Single guided relocation used as a GA mutation, not an iterative local search.
    dd = D
    size = SIZE
...
def route_preserving_child(a: Individual, b: Individual, cfg: Dict[str, Any]) -> Individual:
    ...
    idxs = list(range(K))
    if rng.random() < cfg["copy_route_bias"]:
...
def crossover(p1: Individual, p2: Individual, cfg: Dict[str, Any]) -> Tuple[Individual, Individual]:
    if rng.random() < 1.0 - cfg["order_crossover_rate"]:
...
def mutate(ind: Individual, cfg: Dict[str, Any]) -> Individual:
    routes = [r[:] for r in ind.routes]
    p = rng.random()

    if p < 0.45:
        costs = get_costs(routes)
        changed = guided_relocate_from_longest(routes, costs, cfg["relocate_sample"], cfg["relocate_dest"], cfg["mixed"])
        if changed:
            return evaluate_clean(routes, costs)
...
def solve() -> Individual:
    ...
    while pc() < end_time:
        ...
        # Basic GA diversity injection: replace only the worst part when stuck.
        # This is not local search; it only reintroduces new chromosomes.
